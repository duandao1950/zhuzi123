$(function() {
	...
	var url_action = {url_list:"article_list.action",url_edit:"article_edit.action",
			url_save_or_update:"article_saveOrUpdate.action",url_del:"article_delete.action"};
	...
	function search(){
		$.ajax({type: "POST",url: url_action.url_list,data:"",dataType:"json",
		    ...
		    for(var j=0;j<cellVaues.length;j++){
				...
				trStr += "<td>"
				trStr += "<ul id='icons' class='ui-widget ui-helper-clearfix'>";
			    trStr += "<li class='ui-state-default ui-corner-all' title='Edit'>";
				trStr += "<span class='ui-icon ui-icon-wrench' onclick='$.editAjax("+tmp_operator_id+");'/>"
				trStr += "<li/>";
				trStr += "<li class='ui-state-default ui-corner-all' title='Delete'>";
				trStr += "<span class='ui-icon ui-icon-trash' onclick='$.deleteAjax("+tmp_operator_id+");'/>"
				trStr += "<li/>";
				trStr += "<ul/>";
				trStr += "</td>"
				trStr += "</tr>";
	...
	jQuery.extend({
			...
			$.ajax({
			    ...
			    success:function(text){
			    	...
			    	if (text != null){
			    		...
			    		if(jQuery.isNull(jsonObject)){
			    	   	    alert("数据为空,无法为表格赋值!");
			    	   	    return;
			    	   	}
	
			    		for (var i = 0; i< formInput.length; i++){
			    		    if (!jQuery.isNull(document.getElementById(formInput[i]))){
			    		    	document.getElementById(formInput[i]).value = jQuery.isNullToString(jsonObject[formInput[i]]);
			    		    }
			    		}
			    	}
			    }
		...
		isNullToString: function isNullToString(obj){
			if(obj == null || $.trim(obj) == "" || typeof(obj) == "undefined"){
				return " ";
			}
			return obj;
